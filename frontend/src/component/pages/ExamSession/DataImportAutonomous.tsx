...
import { Upload, FileSpreadsheet, ArrowLeft, ArrowRight, Trash2, AlertCircle } from "lucide-react"
...
export function DataImportAutonomous({ onUpload, onBack }: { onUpload: (data: any) => void; onBack: () => void }) {
  ...
  const updateTag = (id: string, field: keyof AutoFileEntry, value: string) => {
    setFileEntries(prev => prev.map(entry => {
      ...
      // Auto-update Course Code when Subject changes
      ...
      <CardContent className="space-y-6 pt-6">
        ...
        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
          {fileEntries.map((entry) => {
            const subjects = getSubjects(entry.batch, entry.semester, entry.dept);
              ...
              <div key={entry.id} className="p-5 border rounded-xl bg-white shadow-sm space-y-4 border-slate-100">
                <div className="flex items-center justify-between border-b pb-3 border-slate-50">
                  <div className="flex items-center gap-3 text-blue-600">
                    ...
                    <span className="text-sm font-black truncate max-w-md uppercase tracking-tight">
                      {entry.file.name}
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select 
                      className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-xs font-bold focus:ring-2 focus:ring-blue-600 outline-none"
                      value={entry.batch} 
                  ...
                  <div className="space-y-1.5 md:col-span-2">
                    ...
                    <select 
                      ...
                      disabled={!entry.batch || !entry.semester || !entry.dept || !subjects.length}
                    >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input 
                      ...
                      readOnly 
                      className="bg-slate-50 border-slate-200 font-black text-blue-700 text-xs h-10" 
                    />
                  </div>
                </div>
              </div>
