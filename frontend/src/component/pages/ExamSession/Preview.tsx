...
export default function Preview({
  ...
  const collectFiles = () => {
    if (payload.data?.length) {
      return payload.data.map((item: any) => item.file).filter(Boolean);
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/v1/upload/students`, {
        method: "POST",
        body: formData,
      });
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Verify file mappings before generation
            </p>
          </div>
          <Badge
            variant="outline"
            className="bg-background px-3 py-1 capitalize font-bold"
          >
      ...
      <CardContent className="py-6 space-y-6">
          ...
          {payload.data
            ? payload.data.map((item: any, i: number) => (
                ...
                >
                  ...
                  <div
                    className={`p-5 grid grid-cols-1 md:grid-cols-${isInternal ? 3 : 1} gap-8`}
                  >
        ...
        <div className="flex justify-between items-center pt-6 border-t mt-8">
          ...
          <div className="flex gap-3">
            ...
            >
              {isUploading ? "Uploading..." : "Confirm & Generate"}
            </Button>
          </div>
        </div>
      </CardContent>
